...
export function BlackjackTable() {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 flex-shrink-0" style={{ borderBottom: '0.5px solid rgba(255,255,255,0.04)' }}>
        <div>
        ...
        {phase === 'bet' && (
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1.5">
              <div className="h-9 rounded-lg bg-white/[0.03] border border-white/[0.06] flex items-center justify-center px-3">
              ...
              <div className="flex gap-1">
                {[1000, 5000, 10000, 25000, 50000].map(a => (
                  <button key={a} onClick={() => setBetAmount(a)}
                    className={`h-7 px-2 rounded-lg text-[9px] font-mono font-medium transition-all cursor-pointer ${betAmount === a ? 'bg-white text-black' : 'bg-white/[0.03] text-white/50 border border-white/[0.06] hover:text-white hover:bg-white/[0.06]'}`}
                  >{formatNum(a)}</button>
                ))}
              </div>
            </div>
          ...
          {splitHands.length > 0 ? (
            <div className="space-y-4">
              {splitHands.map((hand, i) => {
                ...
                return (
                  <div key={i} className={`p-3 rounded-xl ${isActive ? 'ring-1 ring-yellow-400/30' : ''}`}
      ...
      <div className="flex-shrink-0 px-4 py-3" style={{ borderTop: '0.5px solid rgba(255,255,255,0.04)' }}>
        {phase === 'bet' && (
